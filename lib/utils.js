...
function getDefaultLastUrl(options) {
  return options.defaultLastUrl || '/'
}

function getLastUrl(req, options) {
  var lastUrl = (req.session && req.session.lastUrl) ? req.session.lastUrl : getDefaultLastUrl(options);

  var uri = url.parse(lastUrl);

  if (uri.pathname === options.paths.validate) lastUrl = getDefaultLastUrl(options);
...
function sendRequest(path, options, callback) {
  ...
  var req = (requestOptions.protocol === 'https:' ? https : http )['request'](requestOptions, function(res) {
